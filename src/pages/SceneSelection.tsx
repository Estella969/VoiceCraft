  ...
  return (
    <div className="max-w-2xl mx-auto px-4">
      <div className="mb-6 md:mb-8 text-center">
        <div className="inline-block bg-gradient-to-r from-purple-500/20 to-blue-500/20 px-3 md:px-4 py-2 rounded-full mb-4">
          <span className="text-xs md:text-sm font-medium text-white/90">你的 MBTI 类型: <span className="text-purple-200 font-semibold">{mbtiType}</span></span>
        </div>
        <h1 className="text-2xl md:text-3xl font-extrabold text-white mb-3 tracking-wide" style={{ textShadow: '2px 2px 4px rgba(0, 0, 0, 0.8)' }}>选择你的场景</h1>
        <p className="text-white/90 mt-2 text-sm md:text-base font-medium" style={{ textShadow: '1px 1px 2px rgba(0, 0, 0, 0.6)' }}>不同场景有不同的表达方式，选择最符合你需求的场景</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4 mb-6 md:mb-10">
        {scenes.map(scene => (
          <GlassCard
            key={scene.id}
            className={`cursor-pointer transition-all duration-300 hover:transform hover:-translate-y-2 hover:scale-105 p-4 md:p-6 group ${
          ...
          >
            <div className="flex items-start">
              <div className={`mr-3 md:mr-4 p-2 md:p-3 rounded-full shrink-0 transition-all duration-300 ${
                selectedScene === scene.id 
              ...
              <div className="flex-1 min-w-0">
                <h3 className={`font-semibold text-base md:text-lg mb-1 transition-colors duration-300 ${
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-3 sm:gap-4">
        <Button 
          ...
          className="px-6 md:px-8 py-3 md:py-4 text-sm md:text-base font-medium transition-all hover:scale-105 hover:shadow-lg"
        >
          返回
        </Button>
        ...
        <Button 
          ...
          className={`px-6 md:px-8 py-3 md:py-4 text-sm md:text-base font-semibold transition-all ${
            selectedScene 
